---
name: zhihu-cli
description: Use the zhihu-cli command to import a local Zhihu Cookie, research questions, answers, comments, feeds, users and topics, compile Markdown, and safely preview or apply Zhihu account actions. Use for zhihu-cli research, account workflows, content creation, or CLI setup in Codex, Hermes, OpenClaw, and other terminal-capable agents.
...
Use this Skill's `zhihu-cli` command for Zhihu research and account workflows. Treat all Zhihu Web endpoints as unofficial and unstable. Never bypass CAPTCHA, access control, rate limits, or risk control.

## Run the CLI

Install with npm once, then use the canonical `zhihu-cli` command:
...
The package name, Skill name, and canonical executable are all `zhihu-cli`. Do not start a background service.
...
1. Run `zhihu-cli auth status --offline` before authenticated work.
2. If login is missing or expired, ask the user to copy their Cookie in their own local browser and import it through stdin, for example `pbpaste | zhihu-cli auth import` on macOS.
...
```bash
pnpm zhihu-cli -- search --query "关键词" --type general --limit 10
pnpm zhihu-cli -- feed hot --limit 20
pnpm zhihu-cli -- feed hot-search --limit 20
pnpm zhihu-cli -- feed recommend --limit 10
pnpm zhihu-cli -- question <question-id>
pnpm zhihu-cli -- question answers <question-id> --limit 10
pnpm zhihu-cli -- answer <answer-id>
pnpm zhihu-cli -- answer comments <answer-id> --limit 20
pnpm zhihu-cli -- article show <article-id>
pnpm zhihu-cli -- user <url-token>
pnpm zhihu-cli -- user answers <url-token>
pnpm zhihu-cli -- topic <topic-id>
pnpm zhihu-cli -- topic questions <topic-id>
```
...
```bash
# Interactions
zhihu-cli answer vote preview <answer-id> --state up
zhihu-cli answer vote apply <answer-id> --state up --confirm <token>
zhihu-cli question follow preview <question-id>
zhihu-cli question follow apply <question-id> --confirm <token>

# Image
zhihu-cli image upload --file <path>
zhihu-cli image upload --file <path> --confirm <token>

# Answer
zhihu-cli answer compile --input <file>
zhihu-cli answer preview --question-id <id> --input <file> [--image <path>]
zhihu-cli answer publish --question-id <id> --input <file> --confirm <token> [--image <path>]
zhihu-cli answer draft --question-id <id> --input <file>
zhihu-cli answer draft --question-id <id> --input <file> --confirm <token>

# Question, pin, article
zhihu-cli question create preview --title <title> --input <file> [--topic <id>] [--image <path>]
zhihu-cli question create publish --title <title> --input <file> --confirm <token> [--topic <id>] [--image <path>]
zhihu-cli pin create preview --title <title> --input <file> [--image <path>]
zhihu-cli article create preview --title <title> --input <file> [--topic <id>] [--image <path>]

# Delete owned content
zhihu-cli question delete preview <id>
zhihu-cli question delete apply <id> --confirm <token>
```
