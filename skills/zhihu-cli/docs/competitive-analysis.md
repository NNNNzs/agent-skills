# zhihu-cli 功能对比与设计决策记录

> 状态：本文是公开资料形成的历史设计记录，不是当前功能清单或未来路线图。功能与命令以 README、SKILL.md 和测试为准；外部项目会持续变化，读者应自行复核链接与提交快照。

> 登录现状：本文保留二维码能力的历史分析，但当前知乎二维码接口返回 HTTP 403。面向用户与 Agent 的实际登录流程以本机 stdin 粘贴导入 Cookie 为准，不应转发二维码、二维码 URL 或 token。

## 1. 分析对象与结论

- 对比对象：[`BAIGUANGMEI/zhihu-cli`](https://github.com/BAIGUANGMEI/zhihu-cli)，仅使用公开仓库资料。
- 可复核快照：[`8e32b99`](https://github.com/BAIGUANGMEI/zhihu-cli/tree/8e32b99)。
- 观察到的版本标记：`pyproject.toml` 与 `zhihu_cli/__init__.py` 为 `0.2.4`，`skill/SKILL.md` 为 `0.2.6`；仅表示该快照中的信息。
- 分析目标：`NNNNzs/zhihu-cli` 的公开功能设计，不复制对比项目的代码、凭证传递方式或写操作安全模型。

在该历史快照中，对比项目的 CLI 广度更大，共注册 27 个子命令，覆盖扫码登录、用户、评论、话题、互动、收藏、通知、提问、想法、文章及删除；本项目的回答创作链路则包含 Markdown 编译、回答草稿、发布预览、确认令牌、回答创建/更新和 GIF 图片上传。
...
| 能力 | Python 竞品 | 当前项目 | 差距与建议 |
|---|---|---|---|
| 全局可执行命令 | `zhihu` | package bin 为 `zhihu-cli`，开发时 `pnpm zhihu-cli` | 当前项目已统一使用 `zhihu-cli` |
...
7. 命令采用 `zhihu-cli auth login --qr`，不提供旧的短命令别名；输出事件应为 JSON Lines 或一个最终 JSON，明确 `qr_ready`、`scanned`、`confirmed`、`expired` 状态。
...
> 面向人类与 AI Agent 的完整 `zhihu-cli`：支持安全登录、内容检索与阅读、用户和话题研究、互动、Markdown 创作、多类型内容发布与受确认保护的写操作。

当前项目的核心差异化不是“比竞品多几个命令”，而是把 `zhihu-cli` 的广度与回答创作链路的安全性结合起来。
